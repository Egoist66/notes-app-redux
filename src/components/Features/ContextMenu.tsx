...
export const ContextMenu: FC<ContextMenuProps> = ({
  ...
  const increaseTextFont = () => {
    return () => {
      ...
      transformText(
        `<span style="font-size:${fontCounter}px">${strippedText}</span>`
      );
    };
  };
  ...
  return (
    <StyledContextOverlay id="context-overlay">
      <StyledContextMenu id="context-menu">
        <p onClick={() => transformText(`<b>${strippedText}</b>`)}>
          Сделать жирным текст
        </p>
        <p onClick={() => transformText(`<i>${strippedText}</i>`)}>
          Сделать курсивным текст
        </p>
        <p onClick={() => transformText(`<mark>${strippedText}</mark>`)}>
          Сделать выделенным текст
        </p>
